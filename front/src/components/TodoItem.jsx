...
class TodoItem extends Component {
    ...
    render() {
        ...
        const rightIconMenu = (
            <IconMenu iconButtonElement={< IconButton > <MoreVertIcon color={Styles.Colors.grey400}/> < /IconButton>}>
        ...
        if (this.state.editing) {
            ...
            element = (
                <ListItem primaryText={todo.text} onTouchTap={() => completeTodo(todo.id)} leftIcon={todo.completed
                    ? <CheckBoxIcon/>
                    : <CheckBoxBlankIcon/>} rightIconButton={rightIconMenu}/>
            );
        }

        return (
            <div className={classnames({completed: todo.completed, editing: this.state.editing})}>
                {element}
            </div>
        );
    }
