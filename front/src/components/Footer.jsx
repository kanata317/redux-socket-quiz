...
class Footer extends Component {
    ...
    renderFilterLink(filter) {
        ...
        return (<ListItem key={filter} className={classnames({selected: active})} style={{
            color: active
                ? palette.primary1Color
                : palette.textColor
        }} primaryText={title + (count > 0
            ? ' (' + count + ')'
            : '')} leftIcon={FILTER_ICONS[filter]} onTouchTap={() => onShow(filter)}/>);
    }

    renderClearButton() {
        const {completedCount, onClearCompleted} = this.props;
        if (completedCount > 0) {
            return (<RaisedButton className="clear-completed" primary={true} label="Clear completed" onClick={onClearCompleted}/>);
        }
    }
